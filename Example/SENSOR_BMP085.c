...
BMP085_CAL_COFF BMP085_CAL_COFF_DATA;
...
void BMP085_GET_CAL_COEFF(void)
{
       ...
       CHAR8S fail_flag=0;
		...
		for(count=0;count<CAL_COEFF_NUMBER;count++)
	        ...
	        BMP085_CAL_COFF_DATA.AC1 = (INT16S) ( (BMP_CAL_COFF_ARRAY[0]<<8) + BMP_CAL_COFF_ARRAY[1]);
	        BMP085_CAL_COFF_DATA.AC2 = (INT16S) ( (BMP_CAL_COFF_ARRAY[2]<<8) + BMP_CAL_COFF_ARRAY[3]);
	        BMP085_CAL_COFF_DATA.AC3 = (INT16S) ( (BMP_CAL_COFF_ARRAY[4]<<8) + BMP_CAL_COFF_ARRAY[5]);
	        BMP085_CAL_COFF_DATA.AC4 = (INT16S) ( (BMP_CAL_COFF_ARRAY[6]<<8) + BMP_CAL_COFF_ARRAY[7]);
	        BMP085_CAL_COFF_DATA.AC5 = (INT16S) ( (BMP_CAL_COFF_ARRAY[8]<<8) + BMP_CAL_COFF_ARRAY[9]);
	        BMP085_CAL_COFF_DATA.AC6 = (INT16S) ( (BMP_CAL_COFF_ARRAY[10]<<8) + BMP_CAL_COFF_ARRAY[11]);
	        BMP085_CAL_COFF_DATA.B1 =  (INT16S) ( (BMP_CAL_COFF_ARRAY[12]<<8) + BMP_CAL_COFF_ARRAY[13]);
	        BMP085_CAL_COFF_DATA.B2 =  (INT16S) ( (BMP_CAL_COFF_ARRAY[14]<<8) + BMP_CAL_COFF_ARRAY[15]);
	        BMP085_CAL_COFF_DATA.MB =  (INT16S) ( (BMP_CAL_COFF_ARRAY[16]<<8) + BMP_CAL_COFF_ARRAY[17]);
	        BMP085_CAL_COFF_DATA.MC =  (INT16S) ( (BMP_CAL_COFF_ARRAY[18]<<8) + BMP_CAL_COFF_ARRAY[19]);
	        BMP085_CAL_COFF_DATA.MD =  (INT16S) ( (BMP_CAL_COFF_ARRAY[20]<<8) + BMP_CAL_COFF_ARRAY[21]);
		...
		printf("AC1 = 0x%X(%d)\r\n", BMP085_CAL_COFF_DATA.AC1,BMP085_CAL_COFF_DATA.AC1);
		printf("AC2 = 0x%X(%d)\r\n", BMP085_CAL_COFF_DATA.AC2,BMP085_CAL_COFF_DATA.AC2);
		printf("AC3 = 0x%X(%d)\r\n", BMP085_CAL_COFF_DATA.AC3,BMP085_CAL_COFF_DATA.AC3);
		printf("AC4 = 0x%X(%d)\r\n", BMP085_CAL_COFF_DATA.AC4,BMP085_CAL_COFF_DATA.AC4);
		printf("AC5 = 0x%X(%d)\r\n", BMP085_CAL_COFF_DATA.AC5,BMP085_CAL_COFF_DATA.AC5);
		printf("AC6 = 0x%X(%d)\r\n", BMP085_CAL_COFF_DATA.AC6,BMP085_CAL_COFF_DATA.AC6);
		printf("B1 = 0x%X(%d)\r\n", BMP085_CAL_COFF_DATA.B1,BMP085_CAL_COFF_DATA.B1);
		printf("B2 = 0x%X(%d)\r\n", BMP085_CAL_COFF_DATA.B2,BMP085_CAL_COFF_DATA.B2);
		printf("MB = 0x%X(%d)\r\n", BMP085_CAL_COFF_DATA.MB,BMP085_CAL_COFF_DATA.MB);
		printf("MC = 0x%X(%d)\r\n", BMP085_CAL_COFF_DATA.MC,BMP085_CAL_COFF_DATA.MC);
		printf("MD = 0x%X(%d)\r\n", BMP085_CAL_COFF_DATA.MD,BMP085_CAL_COFF_DATA.MD);
...
CHAR8S BMP085_GET_TEMP_PRESS(INT32S *temperature_data,FLOAT *pressure_data,CHAR8U oss)
{
	...
	FLOAT  p_float;
		...
		if(fail_flag == SWI2C_STATUS_FAIL)  
		{
        	...
        	delay_ms(6);	/* wait for BMP085 conversion is finished  see spec page.17 */ 
		...
		fail_flag = i2c_write_1_byte_data(BMP085_SLAVE_ADDRESS,0XF4, 0X34 + (CHAR8U)(oss<< 6) );  /* oss -> set the BMP085 measurement pressure mode */
		if(fail_flag == SWI2C_STATUS_FAIL)  
		{
			return -1;
		}

		switch(oss)	/* wait for BMP085 conversion is finished  see spec page.17 */ 
        	{
		...
		X1 = ((INT32S)Temp_value - BMP085_CAL_COFF_DATA.AC6)*(BMP085_CAL_COFF_DATA.AC5)/32768;
		X2 =  (INT32S)((INT32S)BMP085_CAL_COFF_DATA.MC *2048)/(INT32S)(X1 + (INT32S)BMP085_CAL_COFF_DATA.MD);
		B5 = (INT32S)(X1 + X2);
		REAL_TEMPUTRE = (INT32S)(B5 + 8)/ 16;        /* Final Temperature , ex: 153 temp  setp in 0.1 , 15.3 oC */
       	...
       	pr1 = (INT32S)high_byte_press << 16;
        	...
        	X1 = ((BMP085_CAL_COFF_DATA.B2)*pr1)/2048 ;
        	X2 = (INT32S) ((INT32S)BMP085_CAL_COFF_DATA.AC2) * B6 / 2048;
        	X3 = X1 + X2;
        	B3 =(INT32S)(((((INT32S)BMP085_CAL_COFF_DATA.AC1*4+X3)<< (INT32S)(oss) ) + 2)/4 );
        	X1 =(INT32S)(((INT32S)BMP085_CAL_COFF_DATA.AC3 * B6)/8192);
        	X2 =(INT32S)(((INT32S)BMP085_CAL_COFF_DATA.B1 * ((B6*B6)/4096))/65536);
        	X3 =(INT32S)(((X1 + X2) + 2)/4);
        	pr4 =(INT32S)(X3 + 32768);
        	pr5 =(INT32S)BMP085_CAL_COFF_DATA.AC4;
